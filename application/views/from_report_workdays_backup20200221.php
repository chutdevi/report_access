...
#@SHEET 5
	...
	$FJ->STYLE_HEIGHT( 4, 8 );	
	$FJ->STYLE_HEIGHT( 6, 8 );
	$FJ->STYLE_HEIGHT( 7, 25);

	$FJ->CREATE_FILL('B2' , $cx[$ac+1].($ad+1), '31869b' );
	$FJ->CREATE_FILL('D3' , 'D3', '632523' );
	$FJ->CREATE_FILL('C8' , 'D8', '366092' );
	$FJ->CREATE_FILL('G8' , 'M8', '7030a0' );

	$FJ->CREATE_FONT( $cx[ $cn["B"] ] . '2', $cx[ $cn["N"] ] . $ad    , 11, 'fde9d9', true );
	$FJ->CREATE_FONT( $cx[ $cn["D"] ] . '3', $cx[ $cn["D"] ] . '3'    , 11, 'FFFF00', true );
	...
	$FJ->CREATE_TEXT( $cx[ $cn["D"] ] . '3', "Max day" );
	...
	foreach( range(-3,3) as $x => $v )
		{
				...
				$FJ->CREATE_TEXT( $cx[ $cn["G"]+$x ] . '5', $runsYears );
				if( $thisMonth > $runsMonth)
					{
						$FJ->CREATE_TEXT( $cx[ $cn["G"]+$x ] . '8', "(actual)" );
						$FJ->CREATE_FONT( $cx[ $cn["G"]+$x ] . '8', $cx[ $cn["G"]+$x ] . '8' , 11, 'FFFF00', true );			
					} 
				else
					{
						$FJ->CREATE_TEXT( $cx[ $cn["G"]+$x ] . '8', "(plan)" );
						$FJ->CREATE_FONT( $cx[ $cn["G"]+$x ] . '8', $cx[ $cn["G"]+$x ] . '8' , 11, '66FF99', true );	
					}
				if($y != $runsYears)
					{
						$FJ->CREATE_FILL($cx[ $cn["G"] ] . '5',$cx[ $cn["G"]+($x-1) ] . '5', '4f6228' );
						$FJ->CREATE_MERGECELL( $cx[ $cn["G"] ] . '5',$cx[ $cn["G"]+($x-1) ] . '5');	
						$y  = $runsYears;
						$ty = $cn["G"] + $x ; 	
					}	
		}

	$FJ->CREATE_FILL( $cx[ $ty ] . '5', $cx[ $ac ] . '5', '244062' );
	$FJ->CREATE_MERGECELL( $cx[ $ty ] . '5', $cx[ $ac ] . '5' );	
