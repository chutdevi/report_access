...
function checklogin() {
    ...
    postData('http://192.168.161.102/api_system/Api_event_reportaccess/evtsys_eventlogin', { user: usn, pass: MD5(pwd.trim()) })
        .then((data) => {
            ...
            if (data.msg == "0") document.querySelector('.notilogin>p').innerHTML = '';
            setTimeout(function() {
                if (data.msg == "0") document.querySelector('.notilogin>p').innerHTML = '<i class="fa fa-frown-o ih"></i> Please enter the correct information. There is a wrong username or password.'
            }, 200);
            let timeOut = (data.msg == "0") ? 4000 : 10000;
            setTimeout(function() {
            ...
            document.querySelector('.notilogin>p').innerHTML = '';
            setTimeout(function() {
                document.querySelector('.notilogin>p').innerHTML = '<i class="fa fa-frown-o ih"></i> System error!. Please try agin.';
            }, 200);
            setTimeout(function() {
                document.querySelector(".notilogin").classList.add("notihide")
                setTimeout(function() {
